...
namespace ModelLayer.Model.Quotation.MultiPanel
{
    public class MultiPanelModel : IMultiPanelModel, INotifyPropertyChanged
    {
        ...
        public void SetDimensions_PanelObjs_of_3rdLevelMPanel(int divmovement, string prevOrNxt)
        {
            if (MPanel_ParentModel != null)
            {
                if (MPanel_Type == "Mullion") // ------------------- Mullion
                {
                    if (MPanel_ParentModel.MPanel_Placement == "First")
                    {
                        foreach (IPanelModel pnl in MPanelLst_Panel)
                        {
                            if (pnl.Panel_Placement == "Last")
                            {
                                ...
                                //{
                                //    pnl.Panel_WidthToBind = ((int)(pnl.Panel_Width * pnl.Panel_Zoom));
                                //    int totalWd_panelModel = MPanelLst_Panel.Sum(pnls => pnls.Panel_WidthToBind + pnls.Panel_MarginToBind.Left + pnls.Panel_MarginToBind.Right),
                                ...
                                //    {
                                //        pnl.Panel_WidthToBind += diff_MPanelWd_VS_MyCtrlsWidth;
                                //    }
                        ...
                        foreach (IPanelModel pnl in MPanelLst_Panel)
                        {
                            if (prevOrNxt == "nxt")
                            {
                                if (pnl.Panel_Placement == "First")
                                {
                                    ...
                                    //    int totalWidth_Controls = totalWd_panelModel + totalWd_divModel + totalWd_MpanelModel - 1;
                                    //    int diff_MPanelWd_VS_MyCtrlsWidth = MPanel_WidthToBind - totalWidth_Controls;
                ...
                else if (MPanel_Type == "Transom") // ------------------- Transom
                {
                    if (MPanel_ParentModel.MPanel_Placement == "First")
                    {
                        foreach (IPanelModel pnl in MPanelLst_Panel)
                        {
                            if (pnl.Panel_Placement == "Last")
                            {
                                ...
                                //else if (MPanel_Zoom == 0.50f)
                                //{
                                //    pnl.Panel_HeightToBind = ((int)(pnl.Panel_Height * pnl.Panel_Zoom));
                                //    int totalHt_panelModel = MPanelLst_Panel.Sum(pnls => pnls.Panel_HeightToBind + pnls.Panel_MarginToBind.Top + pnls.Panel_MarginToBind.Bottom),
                                ...
                                //    {
                                //        pnl.Panel_HeightToBind += diff_MPanelHt_VS_MyCtrlsHeight;
                                //    }
                        ...
                        foreach (IPanelModel pnl in MPanelLst_Panel)
                        {
                            if (prevOrNxt == "nxt")
                            {
                                if (pnl.Panel_Placement == "First")
                                {
                                    ...
                                    //    int totalHeight_Controls = totalHt_panelModel + totalHt_divModel + totalHt_MpanelModel - 1;
                                    //    int diff_MPanelHt_VS_MyCtrlsHeight = MPanel_HeightToBind - totalHeight_Controls;
                                    ...
                                    //    else
                                    //    {
                                    //        Console.WriteLine(MPanel_HeightToBind - totalHeight_Controls + 1);
                                    //    }
                                    //}
                                    else if (MPanel_Zoom <= 0.50f)
                                    {
            ...
            if (MPanel_ParentModel != null)
            {
                ...
                if (MPanel_Type == "Mullion")
                {
                    foreach (IPanelModel pnl in MPanelLst_Panel)
                    {
                        ...
                        {
                            int pnlhtToBind = MPanel_HeightToBind - (int)(pnl.Panel_Margin.Top * pnl.Panel_Zoom) - (int)(pnl.Panel_Margin.Bottom * pnl.Panel_Zoom);
                            pnl.Panel_HeightToBind = pnlhtToBind;
                        }
                ...
                else if (MPanel_Type == "Transom")
                {
                    foreach (IPanelModel pnl in MPanelLst_Panel)
                    {
                        ...
                        {
                            int pnlwdToBind = MPanel_WidthToBind - (int)(pnl.Panel_Margin.Left * pnl.Panel_Zoom) - (int)(pnl.Panel_Margin.Right * pnl.Panel_Zoom);
                            pnl.Panel_WidthToBind = pnlwdToBind;
                        }
                        else if (MPanel_Zoom <= 0.26f)
                        {
                            //int pnlwdToBind = MPanel_WidthToBind - pnl.Panel_MarginToBind.Left - pnl.Panel_Margin.Right;
        ...
        public void SetDimensionsToBind_using_ParentMultiPanelModel()
        {
            ...
            else if (MPanel_Zoom == 0.50f)
            {
                ...
                else if (MPanel_ParentModel.MPanel_Type == "Transom")
                {
                    ...
                    decimal ht_dec = decimal.Round(ht_flt_convert_dec / 2, 0, MidpointRounding.AwayFromZero) * 2;
                    wd = parent_wdToBind;
                    ht = (int)(MPanel_Height * MPanel_Zoom);
                }
            }
            else if (MPanel_Zoom == 1.0f)
            {
        ...
        public void Imager_SetDimensionsToBind_using_ParentMultiPanelModel_Initial()
        {
            int parent_wdToBind = MPanel_ParentModel.MPanelImageRenderer_Width,
                parent_htToBind = MPanel_ParentModel.MPanelImageRenderer_Height,
                totalpanel_inside_parentMpanel = MPanel_ParentModel.MPanel_Divisions + 1,
                totalObject = MPanel_ParentModel.MPanelLst_Panel.Count + MPanel_ParentModel.MPanelLst_MultiPanel.Count,
                div_count = MPanel_ParentModel.MPanel_Divisions,
            ...
            if (MPanelImageRenderer_Zoom == 1.0f || MPanelImageRenderer_Zoom == 0.50f)
            {
                if (MPanel_ParentModel.MPanel_Type == "Mullion")
                {
                    int totalWidth_Imagers = MPanel_ParentModel.MPanelLst_Panel.Sum(pnl => pnl.PanelImageRenderer_Width) +
                                             MPanel_ParentModel.MPanelLst_MultiPanel.Sum(mpnl => mpnl.MPanelImageRenderer_Width);
                    ...
                    {
                        wd = ((parent_wdToBind - totalWidth_Imagers) - ((Convert.ToInt32(divSize * MPanelImageRenderer_Zoom) * div_count))) / ((div_count + 1) - (totalObject - 1));
                    }
                    ht = parent_htToBind;
                }
                else if (MPanel_ParentModel.MPanel_Type == "Transom")
                {
                    int totalHeight_Imagers = MPanel_ParentModel.MPanelLst_Panel.Sum(pnl => pnl.PanelImageRenderer_Height) +
                                              MPanel_ParentModel.MPanelLst_MultiPanel.Sum(mpnl => mpnl.MPanelImageRenderer_Height);
                    wd = parent_wdToBind;
                    if (MPanel_Placement == "First")
                    {
                    ...
                    {
                        ht = ((parent_htToBind - totalHeight_Imagers) - ((Convert.ToInt32(divSize * MPanelImageRenderer_Zoom) * div_count))) / ((div_count + 1) - (totalObject - 1));
                    }
                }
            }
            else if (MPanelImageRenderer_Zoom <= 0.26f)
        ...
        public void SetZoomMPanels()
        {
            foreach (IMultiPanelModel mpnl in MPanelLst_MultiPanel)
            {
                ...
                else
                {
                    ...
                    //mpnl.SetDimensionsToBind_using_ZoomPercentage();
                }
            }
        }
                ...
                if (MPanel_Type == "Mullion")
                {
                    if (pnl.Panel_Parent.Parent.Parent.Name.Contains("Multi")) //if Parent.Parent multi-Panel
                    {
                        if (MPanel_Placement == "First")
                        {
                            ...
                            {
                                pnl_margin = new Padding(10, 10, 0, 8);
                            ...
                            {
                                pnl_margin = new Padding(0, 10, 10, 8);
                                pnl.Panel_Placement = "Last";

                            }
                            else
                            {
                                pnl_margin = new Padding(0, 10, 0, 8);
                        ...
                        else if (MPanel_Placement == "Somewhere in Between") //top margin is 9 because of divider on top (8 + 1)
                        {
                            if (pnl.Panel_Index_Inside_MPanel == 0)
                            {
                                if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Window)
                                {
                                    pnl_margin = new Padding(10, 8, 0, 8);
                                }
                                else if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Door)
                                {
                                    pnl_margin = new Padding(10, 10, 0, 8);
                                     ...
                                     (MPanel_DividerEnabled && pnl.Panel_Index_Inside_MPanel == MPanel_Divisions * 2))
                            {
                                if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Window)
                                {
                                    pnl_margin = new Padding(0, 8, 10, 8);
                                }
                                else if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Door)
                                {
                                    pnl_margin = new Padding(0, 10, 10, 8);
                            ...
                            else
                            {
                                if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Window)
                                {
                                    pnl_margin = new Padding(0, 8, 0, 8);
                ...
                else if (MPanel_Type == "Transom")
                {
                    if (pnl.Panel_Parent.Parent.Parent.Name.Contains("Multi")) //if Parent.Parent multi-Panel
                    {
                        if (MPanel_Placement == "First")
                        {
                            if (pnl.Panel_Index_Inside_MPanel == 0)
                            {
                                pnl_margin = new Padding(10, 10, 8, 0);
                                pnl.Panel_Placement = "First";
                            }
                            else if ((!MPanel_DividerEnabled && pnl.Panel_Index_Inside_MPanel == MPanel_Divisions) ||
                                     (MPanel_DividerEnabled && pnl.Panel_Index_Inside_MPanel == MPanel_Divisions * 2))
                                ...
                                if (MPanel_ParentModel.MPanel_ParentModel == null)
                                {
                                    if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Door)
                                    {
                                        if (MPanel_FrameModelParent.Frame_BotFrameArtNo == BottomFrameTypes._7789 ||
                                            ...
                                            MPanel_FrameModelParent.Frame_BotFrameArtNo == BottomFrameTypes._None)
                                        {
                                            pnl_margin = new Padding(10, 0, 8, 0);
                                        }
                                        else
                                        {
                                            pnl_margin = new Padding(10, 0, 8, 10);
                                        }
                                    }
                                    else
                                    {
                        ...
                        else if (MPanel_Placement == "Last")
                        {
                            if (pnl.Panel_Index_Inside_MPanel == 0)
                            {
                                ...
                                {
                                    pnl_margin = new Padding(8, 10, 8, 0);
                                ...
                                if (MPanel_ParentModel.MPanel_ParentModel == null)
                                {
                                    if (MPanel_FrameModelParent.Frame_Type == FrameModel.Frame_Padding.Door)
                                    {
                                        if (MPanel_FrameModelParent.Frame_BotFrameArtNo == BottomFrameTypes._7789 ||
                                            ...
                                            MPanel_FrameModelParent.Frame_BotFrameArtNo == BottomFrameTypes._None)
                                        {
                                            pnl_margin = new Padding(8, 0, 8, 0);
                                        }
                                        else
                                        {
                                            pnl_margin = new Padding(8, 0, 8, 10);
                                        }
                                    }
                                    else
                                    {
        ...
        public void Fit_DisplayDimensions()
        {
            ...
            if (MPanelLst_Objects.Count() > 0 && (MPanelLst_Objects.Count() >= totalCount_objs_to_accomodate))
            {
                if (MPanel_Type == "Transom")
                {
                    ...
                    int diff_MPanelHt_VS_MyCtrlsHeight = MPanel_DisplayHeight - totalDisplayHeight;
                ...
                {
                    int totalWd_panelModel = MPanelLst_Panel.Sum(pnl => pnl.Panel_DisplayWidth),
                        totalWd_MpanelModel = MPanelLst_MultiPanel.Sum(mpnl => mpnl.MPanel_DisplayWidth);

                    int totalWidth_Controls = totalWd_panelModel + totalWd_MpanelModel;

                    int diff_MPanelWd_VS_MyCtrlsWidth = MPanel_DisplayWidth - totalWidth_Controls;
                    List<Control> Lst_Objects = new List<Control>();

                    Lst_Objects = _mpanelLstObjects;
                    Lst_Objects.Reverse();
                    if (diff_MPanelWd_VS_MyCtrlsWidth > 0)
                    {
                        while (diff_MPanelWd_VS_MyCtrlsWidth > 0)
                        {
                            ...
                            foreach (var mpanelObject in Lst_Objects)
                            {
                                ...
                                foreach (IPanelModel pnl in MPanelLst_Panel)
                                {
                                    if (mpanelObject.Name == pnl.Panel_Name)
                                    {
                                ...
                                foreach (IMultiPanelModel mpnl in MPanelLst_MultiPanel)
                                {
                                    if (mpanelObject.Name == mpnl.MPanel_Name)
                                    {
                                        if (diff_MPanelWd_VS_MyCtrlsWidth > 0)
                                        {
        ...
        public void Fit_MyControls_ToBindDimensions()
        {
            ...
            if (MPanelLst_Objects.Count() > 0 && (MPanelLst_Objects.Count() >= totalCount_objs_to_accomodate))
            {
                if (MPanel_Type == "Transom")
                {
                    ...
                    int totalHeight_Controls = totalHt_panelModel + totalHt_divModel + totalHt_MpanelModel;
                    int diff_MPanelHt_VS_MyCtrlsHeight = MPanel_HeightToBind - totalHeight_Controls;
                    int objectIndex, lowestIndex = 0, highestIndex = MPanelLst_Objects.Count -1;
                    objectIndex = highestIndex;
                    if (diff_MPanelHt_VS_MyCtrlsHeight > 0)
                    {
                        ...
                        while (diff_MPanelHt_VS_MyCtrlsHeight > 0)
                        {
                            ...
                            foreach (IPanelModel pnl in MPanelLst_Panel)
                            {
                                if (MPanelLst_Objects[objectIndex].Name == pnl.Panel_Name)
                                {
                                    pnl.Panel_HeightToBind++;
                                    diff_MPanelHt_VS_MyCtrlsHeight--;
                                    Console.WriteLine(pnl.Panel_Name);
                                    break;
                                }
                            }
                            foreach (IMultiPanelModel mpnl in MPanelLst_MultiPanel)
                            {
                                if (MPanelLst_Objects[objectIndex].Name == mpnl.MPanel_Name)
                                {
                            ...
                            if (objectIndex == highestIndex)
                            {
                                highestIndex--;
                                objectIndex = lowestIndex;
                            }
                            else if (objectIndex == lowestIndex)
                            {
                                lowestIndex++;
                                objectIndex = highestIndex;
                            }
                            ...
                            {
                                lowestIndex = 0;
                                highestIndex = MPanelLst_Objects.Count - 1;
                                objectIndex = highestIndex;
                            }
                        }
                    }
        ...
        public void AdjustPropertyPanelHeight(string objtype, string mode)
        {
            ...
            else if (objtype == "Div")
            {
                ...
                {
                    MPanelProp_Height += constants.div_property_claddingArtNoOptionsHeight;
                }
        ...
        public void Fit_MyControls_ToBindDimensions(IMultiPanelModel prev_mpnl, IMultiPanelModel nxt_mpnl, IPanelModel prev_pnl, IPanelModel nxt_pnl, int movement)
        {
            if (MPanelLst_Objects.Count() > 0 && movement != 0)
            {
                if (MPanel_Type == "Transom")
                {
                    int totalHt_panelModel = MPanelLst_Panel.Sum(pnl => pnl.Panel_HeightToBind + pnl.Panel_MarginToBind.Top + pnl.Panel_MarginToBind.Bottom),
                    ...
                    if (diff_MPanelHt_VS_MyCtrlsHeight > 0)
                    {
                        while (diff_MPanelHt_VS_MyCtrlsHeight > 0)
                        {
                            foreach (var mpnObject in MPanelLst_Objects)
                            {
                                foreach (IPanelModel pnl in MPanelLst_Panel)
                                {
                                    if (diff_MPanelHt_VS_MyCtrlsHeight > 0)
                                    {
                                        if ((prev_pnl == pnl && pnl.Panel_Name == mpnObject.Name) || (nxt_pnl == pnl && pnl.Panel_Name == mpnObject.Name))
                                        {
                                            ...
                                            else if (nxt_pnl == pnl)
                                            {
                                                ...
                                                else if (prev_mpnl != null)
                                                {
                                                    prev_mpnl.MPanel_Height--;
                                                    prev_mpnl.MPanel_DisplayHeight--;
                                                }
                                                movement--;
                                            }
                                        }
                                ...
                                foreach (IMultiPanelModel mpnl in MPanelLst_MultiPanel)
                                {
                                    if (diff_MPanelHt_VS_MyCtrlsHeight > 0)
                                    {
                                        if ((prev_mpnl == mpnl && mpnl.MPanel_Name == mpnObject.Name) || (nxt_mpnl == mpnl && mpnl.MPanel_Name == mpnObject.Name))
                                        {
                    ...
                    if (prev_mpnl != null)
                    {
                        if (MPanel_Zoom == 0.26f || MPanel_Zoom == 0.17f ||
                            MPanel_Zoom == 0.13f || MPanel_Zoom == 0.10f)
                        {
                            prev_mpnl.SetDimensions_childPanelObjs(movement);

                            foreach (IMultiPanelModel mpanel in prev_mpnl.MPanel_ParentModel.MPanelLst_MultiPanel)
                            {

                                if (prev_mpnl.MPanel_Name == mpanel.MPanel_Name)
                                {
                                    mpanel.SetDimensions_childObjs(movement, "prev");
                                    ////mpanel.SetDimensions_childPanelObjs(movement);
                                }
                            }
                        }
                        else
                        {
                            prev_mpnl.SetDimensions_childPanelObjs(movement);
                            foreach (IMultiPanelModel mpanel in prev_mpnl.MPanelLst_MultiPanel)
                            {
                                mpanel.MPanel_HeightToBind = prev_mpnl.MPanel_HeightToBind;
                                mpanel.SetDimensions_PanelObjs_of_3rdLevelMPanel(movement, "prev");
                                mpanel.Imager_SetDimensions_PanelObjs_of_3rdLevelMPanel(movement, "prev");
                            }
                            foreach (IDividerModel div in prev_mpnl.MPanelLst_Divider)
                            {
                                div.Div_Height += movement;
                                div.Div_DisplayHeight += movement;
                                div.SetDimensionsToBind_using_DivZoom();
                                div.SetDimensionsToBind_using_DivZoom_Imager();
                            }
                        }
                        if (prev_mpnl.MPanelImageRenderer_Zoom == 0.26f || prev_mpnl.MPanelImageRenderer_Zoom == 0.17f ||
                            prev_mpnl.MPanelImageRenderer_Zoom == 0.13f || prev_mpnl.MPanelImageRenderer_Zoom == 0.10f)
                        {
                            prev_mpnl.ImagerSetDimensions_childPanelObjs(movement);
                        }
                        else
                        {
                            prev_mpnl.ImagerSetDimensions_childPanelObjs(movement);
                    ...
                    if (nxt_mpnl != null)
                    {
                        if (MPanel_Zoom == 0.26f || MPanel_Zoom == 0.17f ||
                            MPanel_Zoom == 0.13f || MPanel_Zoom == 0.10f)
                        {
                            nxt_mpnl.SetDimensions_childPanelObjs(-movement);
                            foreach (IMultiPanelModel mpanel in nxt_mpnl.MPanel_ParentModel.MPanelLst_MultiPanel)
                            {
                                if (nxt_mpnl.MPanel_Name == mpanel.MPanel_Name)
                                {
                                    mpanel.SetDimensions_childObjs(-movement, "nxt");
                                    //mpanel.SetDimensions_childPanelObjs(-movement);
                                }
                            }
                        }
                        else
                        {
                            ...
                            {
                                mpanel.MPanel_HeightToBind = nxt_mpnl.MPanel_HeightToBind;
                                mpanel.SetDimensions_PanelObjs_of_3rdLevelMPanel(-movement, "nxt");
                                mpanel.Imager_SetDimensions_PanelObjs_of_3rdLevelMPanel(-movement, "nxt");
                            }
                        ...
                        {
                            nxt_mpnl.ImagerSetDimensions_childPanelObjs(-movement);
                        }
                        else
                        {
                            nxt_mpnl.ImagerSetDimensions_childPanelObjs(-movement);
                ...
                else if (MPanel_Type == "Mullion") // MULLLLLLLLLLLLLLLLLLLLLLLLLLLLLIIIIIIIIIIIIOOOOOOOOOOOOOOOOOOOOOOOONNNNNN
                {
                    int totalWd_panelModel = MPanelLst_Panel.Sum(pnl => pnl.Panel_WidthToBind + pnl.Panel_MarginToBind.Right + pnl.Panel_MarginToBind.Left),
                        totalWd_divModel = MPanelLst_Divider.Sum(div => div.Div_WidthToBind),
                        totalWd_MpanelModel = MPanelLst_MultiPanel.Sum(mpnl => mpnl.MPanel_WidthToBind);
                                    ...
                                    if (diff_MPanelWd_VS_MyCtrlsWidth > 0)
                                    {
                                        if ((prev_pnl == pnl && pnl.Panel_Name == mpnObject.Name) || (nxt_pnl == pnl && pnl.Panel_Name == mpnObject.Name))
                                        {
                                            ...
                                            else if (nxt_pnl == pnl)
                                            {
                                                ...
                                                {
                                                    prev_mpnl.MPanel_Width--;
                                                    prev_mpnl.MPanel_DisplayWidth--;
                                                }
                                                movement--;
                                            }
                                        }
                    ...
                    if (prev_mpnl != null)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (IMultiPanelModel mpanel in prev_mpnl.MPanelLst_MultiPanel)
                            {
                                mpanel.MPanel_WidthToBind = prev_mpnl.MPanel_WidthToBind;
                                mpanel.SetDimensions_PanelObjs_of_3rdLevelMPanel(movement, "prev");
                                mpanel.Imager_SetDimensions_PanelObjs_of_3rdLevelMPanel(movement, "prev");
                            }
                            ...
                            foreach (IDividerModel div in prev_mpnl.MPanelLst_Divider)
                            {
                                div.Div_Width += movement;
                                div.Div_DisplayWidth += movement;
                    ...
                    if (nxt_mpnl != null)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (IMultiPanelModel mpanel in nxt_mpnl.MPanelLst_MultiPanel)
                            {
                                mpanel.MPanel_WidthToBind = nxt_mpnl.MPanel_WidthToBind;
                                mpanel.SetDimensions_PanelObjs_of_3rdLevelMPanel(-movement, "nxt");
                                mpanel.Imager_SetDimensions_PanelObjs_of_3rdLevelMPanel(-movement, "nxt");
                            }
                            foreach (IDividerModel div in nxt_mpnl.MPanelLst_Divider)
        ...
        public void Fit_MyControls_ImagersToBindDimensions(IMultiPanelModel prev_mpnl, IMultiPanelModel nxt_mpnl, IPanelModel prev_pnl, IPanelModel nxt_pnl)
        {
            if (MPanelLst_Objects.Count() > 0)
            {
                if (MPanel_Type == "Transom")
                    ...
                    if (diff_MPanelImagerHt_VS_MyImagersHeight > 0)
                    {
                        while (diff_MPanelImagerHt_VS_MyImagersHeight > 0)
                        {
                            ...
                            foreach (IMultiPanelModel mpnl in MPanelLst_MultiPanel)
                            {
                                if (diff_MPanelImagerHt_VS_MyImagersHeight > 0)
                                {
                                    if (prev_mpnl == mpnl || nxt_mpnl == mpnl)
                                    {
                                        mpnl.MPanelImageRenderer_Height++;
                                        diff_MPanelImagerHt_VS_MyImagersHeight--;
                                    }
        ...
        public void SetDimensionsToBind_using_ZoomPercentage()
        {
            int mpnl_wd = 0, 
                mpnl_ht = 0,
                parent_wdToBind = MPanel_ParentModel.MPanel_WidthToBind,
                parent_htToBind = MPanel_ParentModel.MPanel_HeightToBind,
                totalpanel_inside_parentMpanel = MPanel_ParentModel.MPanel_Divisions + 1,
                div_count = MPanel_ParentModel.MPanel_Divisions,
                divSize = 0;
                ...
                MPanel_Zoom == 0.13f || MPanel_Zoom == 0.10f)
            {
                if (MPanel_ParentModel.MPanel_Type == "Mullion")
                {
                    ...
                    if (MPanel_ParentModel.MPanel_DividerEnabled)
                    {
                        ...
                        decimal divMove_dec_times2 = divMove_dec * 2;
                        int divMove_int = Convert.ToInt32(divMove_dec_times2);
                        mpnl_wd = ((parent_wdToBind - (divSize * div_count) - divMove_int) / totalpanel_inside_parentMpanel);
                        if (MPanel_Placement == "Last")
                        {
                            int totalWd_panelModel = MPanel_ParentModel.MPanelLst_Panel.Sum(pnl => pnl.Panel_WidthToBind + pnl.Panel_MarginToBind.Right + pnl.Panel_MarginToBind.Left),
                                                   totalWd_divModel = MPanel_ParentModel.MPanelLst_Divider.Sum(div => div.Div_WidthToBind),
                            ...
                            mpnl_wd = MPanel_ParentModel.MPanel_WidthToBind - totalWidth_Controls;
                        }
                        //if(mpanelSize != 0)
                        //{
                        //    mpanelSize -= 2;
                        //}
                        //mpnl_wd = (parent_wdToBind - (divSize * div_count) - mpanelSize) / (totalpanel_inside_parentMpanel - (MPanel_ParentModel.MPanelLst_MultiPanel.Count - 1));

                    }
                    else
                    {
                        mpnl_wd = Convert.ToInt32(((Math.Floor(MPanel_ParentModel.MPanel_Width * MPanel_ParentModel.MPanel_Zoom) - mpanelSize) / (totalPanelCount - (MPanel_ParentModel.MPanelLst_MultiPanel.Count - 1))));
                    }


                    //wd = (parent_wdToBind - (divSize * div_count)) / totalpanel_inside_parentMpanel; //13 px first, then the deduction will occur on Adapt_sizeToBind_MPanelDivMPanel_Controls() 
                    mpnl_ht = parent_htToBind;
                }
                else if (MPanel_ParentModel.MPanel_Type == "Transom")
                {
                    ...
                    {
                        if (mpnl.MPanel_Name != MPanel_Name)
                            mpanelSize += mpnl.MPanel_HeightToBind;
                    }
                    if (MPanel_ParentModel.MPanel_DividerEnabled)
                    {
                        int div_movement = ((MPanel_ParentModel.MPanel_DisplayHeight / totalpanel_inside_parentMpanel) - MPanel_DisplayHeight);
                        decimal divMove_convert_dec = Convert.ToDecimal(div_movement * MPanel_Zoom);
                        decimal divMove_dec = decimal.Round(divMove_convert_dec / 2, 0, MidpointRounding.AwayFromZero);
                        decimal divMove_dec_times2 = divMove_dec * 2;
                        int divMove_int = Convert.ToInt32(divMove_dec_times2);
                        mpnl_ht = ((parent_htToBind - (divSize * div_count) - divMove_int) / totalpanel_inside_parentMpanel);
                        if (MPanel_Placement == "Last")
                        {
                            int totalWd_panelModel = MPanel_ParentModel.MPanelLst_Panel.Sum(pnl => pnl.Panel_HeightToBind + pnl.Panel_MarginToBind.Top + pnl.Panel_MarginToBind.Bottom),
                                                   totalWd_divModel = MPanel_ParentModel.MPanelLst_Divider.Sum(div => div.Div_HeightToBind),
                                                   totalWd_MpanelModel = MPanel_ParentModel.MPanelLst_MultiPanel.Where(d => d != this).Sum(mpnl => mpnl.MPanel_HeightToBind);

                            int totalHeight_Controls = totalWd_panelModel + totalWd_divModel + totalWd_MpanelModel;

                            mpnl_ht = MPanel_ParentModel.MPanel_HeightToBind - totalHeight_Controls;
                        }
                       
                    }
                    else
                    {
                        mpnl_ht = Convert.ToInt32(((Math.Floor(MPanel_ParentModel.MPanel_Height * MPanel_ParentModel.MPanel_Zoom) - mpanelSize) / (totalPanelCount - (MPanel_ParentModel.MPanelLst_MultiPanel.Count - 1))));
                    }
                    mpnl_wd = parent_wdToBind;
                }
            }
            else
            {
                if (MPanel_ParentModel.MPanel_Type == "Mullion")
                {
                    int panelSize = 0;
                    bool isEqual = true;
                    int displayWdExpected = 0;

                    int totalPanelCount = MPanel_ParentModel.MPanel_Divisions + 1;
                    foreach (IPanelModel pnl in MPanel_ParentModel.MPanelLst_Panel)
                    {
                        panelSize += pnl.Panel_WidthToBind;
                        if(displayWdExpected == 0)
                        {
                            displayWdExpected = pnl.Panel_DisplayWidth;
                        }
                        else
                        {
                            if (displayWdExpected != pnl.Panel_DisplayWidth)
                            {
                                isEqual = false;
                            }
                        }
                    ...
                    foreach (IMultiPanelModel mpnl in MPanel_ParentModel.MPanelLst_MultiPanel)
                    {
                        if (mpnl.MPanel_Name != MPanel_Name)
                        {
                            if (displayWdExpected == 0)
                            {
                                displayWdExpected = mpnl.MPanel_DisplayWidth;
                            }
                            else
                            {
                                if(displayWdExpected != mpnl.MPanel_DisplayWidth)
                                {
                                    isEqual = false;
                                }
                            }
                    ...
                    int parent_mpanelWd = MPanel_ParentModel.MPanel_WidthToBind;
                    int totalWd_divModel = MPanel_ParentModel.MPanelLst_Divider.Sum(div => div.Div_WidthToBind);
                    if (MPanel_ParentModel.MPanel_DividerEnabled)
                    {
                        if ((MPanel_ParentModel.isDisplaySizeEqual() || MPanel_Placement == "First") || (MPanel_Placement == "Last" && isEqual))
                        {
                            mpnl_wd = (int)(MPanel_Width * MPanel_Zoom);
                        }
                        else
                        {
                            
                            
                            if (MPanel_ParentModel.MPanelLst_Objects[MPanel_Index_Inside_MPanel - 2].Name.Contains("Multi"))
                            {
                                mpnl_wd = ((parent_mpanelWd - ((int)(16 * MPanel_Zoom) * 2)) - (totalWd_divModel) - panelSize) / (totalPanelCount - ((MPanel_ParentModel.MPanelLst_Panel.Count + MPanel_ParentModel.MPanelLst_MultiPanel.Count) - 1));
                            }
                            else
                            {
                                mpnl_wd = ((parent_mpanelWd - (int)(20 * MPanel_Zoom)) - ((totalWd_divModel - (int)(8 * MPanel_Zoom)) * div_count) - panelSize) / (totalPanelCount - ((MPanel_ParentModel.MPanelLst_Panel.Count + MPanel_ParentModel.MPanelLst_MultiPanel.Count) - 1));
                            }
                        }
                    }
                    else
                    {
                        if (MPanel_ParentModel.isDisplaySizeEqual() || MPanel_Placement == "First")
                        {
                            mpnl_wd = ((parent_mpanelWd - (int)(20 * MPanel_Zoom))) / totalPanelCount;
                        }
                        else
                        {
                            mpnl_wd = ((parent_mpanelWd - (int)(20 * MPanel_Zoom)) - panelSize) / (totalPanelCount - (MPanel_ParentModel.MPanelLst_Panel.Count - 1));
                        }
                    }
                    mpnl_ht = MPanel_ParentModel.MPanel_HeightToBind;
                }
                else if (MPanel_ParentModel.MPanel_Type == "Transom")
                {
                    int panelSize = 0;
                    bool isEqual = true;
                    int displayHTExpected = 0;
                    int totalPanelCount = MPanel_ParentModel.MPanel_Divisions + 1;
                    foreach (IPanelModel pnl in MPanel_ParentModel.MPanelLst_Panel)
                    {
                        panelSize += pnl.Panel_HeightToBind;
                        if (displayHTExpected == 0)
                        {
                            displayHTExpected = pnl.Panel_DisplayHeight;
                        }
                        else
                        {
                            if (displayHTExpected != pnl.Panel_DisplayHeight)
                            {
                                isEqual = false;
                            }
                        }
                    ...
                    foreach (IMultiPanelModel mpnl in MPanel_ParentModel.MPanelLst_MultiPanel)
                    {
                        if (mpnl.MPanel_Name != MPanel_Name)
                        {
                            ...
                            if (displayHTExpected == 0)
                            {
                                displayHTExpected = mpnl.MPanel_DisplayHeight;
                            }
                            else
                            {
                                if (displayHTExpected != mpnl.MPanel_DisplayHeight)
                                {
                                    isEqual = false;
                                }
                            }
                    ...
                    int parent_mpanelHt = MPanel_ParentModel.MPanel_HeightToBind;
                    divSize = (int)(26 * MPanel_Zoom);
                    int totalHt_divModel = MPanel_ParentModel.MPanelLst_Divider.Sum(div => div.Div_HeightToBind);
                    if (MPanel_ParentModel.MPanel_DividerEnabled)
                    {
                        if ((MPanel_ParentModel.isDisplaySizeEqual() || MPanel_Placement == "First") || (MPanel_Placement == "Last" && isEqual))
                        {
                            mpnl_ht = (int)(MPanel_Height * MPanel_Zoom);
                        }
                        else
                        {
                            if (MPanel_ParentModel.MPanelLst_Objects[0].Name.Contains("Multi"))
                            {
                                mpnl_ht = ((parent_mpanelHt) - (totalHt_divModel) - panelSize) / (totalPanelCount - ((MPanel_ParentModel.MPanelLst_Panel.Count + MPanel_ParentModel.MPanelLst_MultiPanel.Count) - 1));
                            }
                            else
                            {
                                mpnl_ht = ((parent_mpanelHt - (int)(20 * MPanel_Zoom)) - (totalHt_divModel) - panelSize) / (totalPanelCount - ((MPanel_ParentModel.MPanelLst_Panel.Count + MPanel_ParentModel.MPanelLst_MultiPanel.Count) - 1));
                            }
                        }
                    }
                    else
                    {
                        if (MPanel_ParentModel.isDisplaySizeEqual() || MPanel_Placement == "First")
                        {
                            mpnl_ht = ((parent_mpanelHt - (int)(20 * MPanel_Zoom))) / totalPanelCount;
                        }
                        else
                        {
                            mpnl_ht = ((parent_mpanelHt - (int)(20 * MPanel_Zoom)) - panelSize) / (totalPanelCount - (MPanel_ParentModel.MPanelLst_Panel.Count - 1));
                        }
                    }
                    mpnl_wd = MPanel_ParentModel.MPanel_WidthToBind;
                }
            }
            MPanel_WidthToBind = mpnl_wd;
            MPanel_HeightToBind = mpnl_ht;
        }
